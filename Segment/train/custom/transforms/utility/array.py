...
from .. transform import Transform, Randomizable, RandomizableTransform
from .. deprecate_utils import optional_import
from ..type_definitions import DtypeLike, NdarrayOrTensor
from ..enums import TransformBackends
from ..utils import (
    extreme_points_to_image, get_extreme_points,
    map_binary_to_indices, map_classes_to_indices,
    concatenate, in1d, moveaxis, unravel_indices,
    ensure_tuple, look_up_option, min_version,
...
class AsChannelFirst(Transform):
    ...
    Most of the image transformations in ``transforms``
...
class AsChannelLast(Transform):
    ...
    so that transforms can construct a chain with other 3rd party transforms together.
...
class RandLambda(Lambda, RandomizableTransform):
    """
    Randomizable version :py:class:`transforms.Lambda`, the input `func` may contain random logic,
    ...
    For more details, please check :py:class:`transforms.Lambda`.
    """
...
class LabelToMask(Transform):
    ...
    def __call__(
        ...
        if img.shape[0] > 1:
            ...
            if isinstance(img, np.ndarray) or (digit_version(TORCH_VERSION)[:2] >= digit_version("1.8")):
...
class FgBgToIndices(Transform):
    ...
    The main logic is based on :py:class:`transforms.utils.map_binary_to_indices`.
...
class CuCIM(Transform):
    ...
    For randomized transforms (or randomly applying a transform) use :py:class:`transforms.RandCuCIM`.
