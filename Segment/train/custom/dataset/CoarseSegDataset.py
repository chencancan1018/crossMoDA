...
class CoarseSegSampleDataset(DefaultSampleDataset):
    def __init__(
            self,
            dst_list_file,
            data_root,
    ...
    def random_contrast_bright(self, basename, img, mask, bright_bias=(-0.4, 0.5), bright_weight=(-0.5, 0.5)):
        # rand_mask_b = random.uniform(-0.5, 0) # for round 1
        # rand_mask_b = random.uniform(-0.5, 0.4) # for round 2

        # img contrast, brightness
        img_mean = np.mean(img)
        rand_img_b = random.uniform(-0.4, 0.5)
        rand_img_w = random.uniform(0.6, 1.5)
        img_out = (img - img_mean) * rand_img_w + img_mean + rand_img_b
        ...
        if vs_pmin is not None:
            ...
            rand_vs_w = random.uniform(0.6, 1.5)
            rand_vs_b = random.uniform(0.6, 1.5) 
            vs_out = ((vs_cube - vs_cube_mean) * rand_vs_w + vs_cube_mean) * rand_vs_b
            #vs部分 gamma aug
            vs_gamma = random.uniform(0.6, 1.5)
        ...
        if coc_pmin is not None:
            ...
            rand_coc_w = random.uniform(0.6, 1.5)
            rand_coc_b = random.uniform(0.6, 1.5) 
            coc_out = ((coc_cube - coc_cube_mean) * rand_coc_w + coc_cube_mean) * rand_coc_b
            # cochlea部分 gamma aug
            coc_gamma = random.uniform(0.6, 2)  
    ...
    def _load_source_data(self, file_name):
        ...
        with torch.no_grad():
            ...
            # import cv2
            # temp = vol.copy()
            # temp = np.clip((temp + 1) / 2, 0, 1)
            # temp = (255 * temp).astype(np.int16)
            # os.makedirs('./test_out', exist_ok=True)
            # sitk_img = sitk.GetImageFromArray(temp)
            # sitk.WriteImage(sitk_img, './test_out/'+basename[:-4]+'_ori_img.nii.gz')

            # temp = seg.copy()
            # temp = temp.astype(np.uint8)
            # os.makedirs('./test_out', exist_ok=True)
            # sitk_img = sitk.GetImageFromArray(temp)
            # sitk.WriteImage(sitk_img, './test_out/'+basename[:-4]+'_ori_seg.nii.gz')

            # contrast vol and target aug
            vol = self.random_contrast_bright(basename, vol, seg.copy())
            vol = np.clip(vol, -1, 1)

            if random.random() < 0.5:
                cochlea_seg = 1 * (seg.copy() >= 2.5)
                radius = random.choice([2, 3])
                cochlea_seg = binary_erosion(cochlea_seg, structure=np.ones((1,radius,radius)), iterations=1)
                seg[seg >= 2.5] = 0
                seg = seg + cochlea_seg * 3

            # import cv2
            # temp = vol.copy()
            # temp = np.clip((vol + 1) / 2, 0, 1)
            # temp = (255 * temp).astype(np.int16)
            # os.makedirs('./test_out', exist_ok=True)
            # sitk_img = sitk.GetImageFromArray(temp)
            # sitk.WriteImage(sitk_img, './test_out/'+basename[:-4]+'_aug_img.nii.gz')

            # temp = seg.copy()
            # temp = temp.astype(np.uint8)
            # os.makedirs('./test_out', exist_ok=True)
            # sitk_img = sitk.GetImageFromArray(temp)
            # sitk.WriteImage(sitk_img, './test_out/'+basename[:-4]+'_aug_seg.nii.gz')
