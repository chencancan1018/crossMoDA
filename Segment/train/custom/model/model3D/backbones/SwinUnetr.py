...
from .unetr import BasicBlock

class DecodeBlock(nn.Module):
    def __init__(self, in_planes, out_planes, upsample_kernel_size, norm_name):
        super(DecodeBlock, self).__init__()
        self.transp_conv = nn.ConvTranspose3d(in_planes, in_planes, kernel_size=upsample_kernel_size, stride=upsample_kernel_size)
        downsample = nn.Sequential(nn.Conv3d(in_planes + out_planes, out_planes, kernel_size=1, stride=1, bias=False),
                                   nn.BatchNorm3d(out_planes),
                                )
        self.block = BasicBlock(in_planes + out_planes, out_planes, stride=1, norm=norm_name, downsample=downsample)
