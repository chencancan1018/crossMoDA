...
import monai
class MonaiTransform2D():
    def __init__(self, opt):
        aug = list()
        if opt.isTrain:
            # rand_crop = monai.transforms.RandScaleCrop(
            #     roi_scale=(0.8,0.8), 
            #     max_roi_scale=(1.1,1.1),
            # )
            # aug.append(rand_crop)

            rand_flipd_x = monai.transforms.RandFlip(
                prob=0.5, spatial_axis=[0], 
            )
            rand_flipd_y = monai.transforms.RandFlip(
                prob=0.5, spatial_axis=[1], 
            )
            aug.append(rand_flipd_x)
            aug.append(rand_flipd_y)

        # rand_elastic = monai.transforms.Rand2DElastic(
        #     spacing=(30,30),
        #     magnitude_range=(5,6),
        #     prob=0.2,
        #     rotate_range=((-np.pi/18, np.pi/18), (-np.pi/18, np.pi/18)),
        #     shear_range=None,
        #     translate_range=((-5.0,5.0), (-5.0,5.0)),
        #     scale_range=((-0.05,0.05), (-0.05,0.05)),
        #     spatial_size=opt.crop_size,
        #     mode="bilinear",
        #     padding_mode='border',
        # )
        # aug.append(rand_elastic)

        # aug.append(transforms.Resize([opt.crop_size, opt.crop_size], Image.BICUBIC))
        # aug.append(transforms.Normalize((0.5,), (0.5,)))
